...
public class BlueTaskCApi {
    ...
    private static final double CIRCLE_CENTER_OFFSET_CM_Y = 9.5;
    private static final double ARM_BACK_CM = 11.9;
    private static final double ERROR_CM = 5.0;
    private static final double GRASPABLE_ANGLE_DEG = 125.0;
    private static final List<Scalar[]> Blue_HSV_RANGES = List.of(
            new Scalar[]{new Scalar(100, 100, 160), new Scalar(130, 255, 255)},
            new Scalar[]{new Scalar(100, 100, 160), new Scalar(130, 255, 255)}
    );
    ...
    private static final int SLIDE_INTERMEDIATE_POSITION = 16000; // End position for stage 2
    ...
    private static final double SLIDE_EXTEND_TIMEOUT_SECONDS = 1.4; // Timeout for the entire extension
    ...
    public boolean initialize(HardwareMap hwMap, Telemetry tel) {
        if (isInitialized) {
            if (tel != null) tel.addLine("BlueTaskCApi: 已初始化。");
            return true;
        }
    ...
    private void handleMovingSlideState() {
        ...
        // Check current power to avoid Blueundant commands, using a small tolerance
    ...
    private void transitionToActionState(ActionState nextState) {
        if (currentActionState == nextState) return; // Avoid Blueundant transitions
    ...
    private void updateServoTargetsFromVision() {
        ...
        if (currentSelectedCube != null) {
            ...
            double normalizedRotateAngle = Range.clip(adjustedRotateAngle, 0, 180) / 180.0; // Normalize 0-180
            double calculatedRotatePos = SERVO_ROTATE_MAX_POS - normalizedRotateAngle * (SERVO_ROTATE_MAX_POS - SERVO_ROTATE_MIN_POS); // Linear reversed map
            double originalCalcRotatePos = 1.0 - (((0.68 / 180.0) * adjustedRotateAngle) + 0.29);
            targetClawRotateServoPos = Range.clip(originalCalcRotatePos, 0.0, 1.0);

        } else {
            // No cube selected. What should happen?
            // Option 1: Hold last known target positions (current behavior) - prevents jitter
            // Option 2: Revert to initial positions
            // targetInFlipTurnServoPos = IN_FLIP_TURN_INIT_POS;
            // targetClawRotateServoPos = CLAW_ROTATE_INIT_POS;
            // Let's stick with holding the last valid target for now.
        }
    }
    ...
    private void logInfo(String message) {
        if (telemetry != null) {
            telemetry.addLine("BlueTaskCApi: " + message);
            telemetry.update(); // Consider updating less frequently if performance is an issue
        }
        System.out.println("BlueTaskCApi INFO: " + message); // Log to console/logcat
    }

    private void logError(String message) {
        if (telemetry != null) {
            telemetry.addLine("!!! BlueTaskCApi 错误: " + message);
            telemetry.update(); // Update immediately for errors
        }
        System.err.println("!!! BlueTaskCApi 错误: " + message); // Log to error stream
    }
    ...
    private void printConfig() {
        ...
        // Use telemetry.log() for persistent messages if desiBlue, or telemetry.addData for per-loop updates
        telemetry.log().clear(); // Clear previous persistent logs
        telemetry.log().add("===================== BlueTaskCApi 配置 =====================");
    ...
    public static class BLUEDetectionPipeline extends OpenCvPipeline {
        // --- Constants copied from BlueTaskCApi ---
        // Ensure these match the outer class's constants *exactly*
        private static final boolean IMAGE_ROTATE_180 = BlueTaskCApi.IMAGE_ROTATE_180;
        private static final double DOWNSCALE_FACTOR = BlueTaskCApi.DOWNSCALE_FACTOR;
        private static final double ARM_BACK_CM = BlueTaskCApi.ARM_BACK_CM;
        private static final double ERROR_CM = BlueTaskCApi.ERROR_CM;
        private static final double CIRCLE_CENTER_OFFSET_CM_X = BlueTaskCApi.CIRCLE_CENTER_OFFSET_CM_X;
        private static final double CIRCLE_CENTER_OFFSET_CM_Y = BlueTaskCApi.CIRCLE_CENTER_OFFSET_CM_Y;
        private static final double GRASPABLE_ANGLE_DEG = BlueTaskCApi.GRASPABLE_ANGLE_DEG;
        private static final double PIXELS_TO_CM_RATIO = BlueTaskCApi.PIXELS_TO_CM_RATIO;
        private static final double MIN_SIZE_PIXELS = BlueTaskCApi.MIN_SIZE_PIXELS;
        private static final double MAX_SIZE_PIXELS = BlueTaskCApi.MAX_SIZE_PIXELS;
        private static final List<Scalar[]> BLUE_HSV_RANGES = BlueTaskCApi.Blue_HSV_RANGES; // Name adjusted for clarity, uses Blue ranges
        private static final Scalar CIRCLE_COLOR = BlueTaskCApi.CIRCLE_COLOR;
        private static final int CIRCLE_THICKNESS = BlueTaskCApi.CIRCLE_THICKNESS;
        private static final Scalar PERSISTENT_LINE_COLOR = BlueTaskCApi.PERSISTENT_LINE_COLOR;
        private static final int PERSISTENT_LINE_THICKNESS = BlueTaskCApi.PERSISTENT_LINE_THICKNESS;
        private static final Scalar BOX_COLOR_SELECTED = BlueTaskCApi.BOX_COLOR_SELECTED;
        private static final Scalar BOX_COLOR_DEFAULT = BlueTaskCApi.BOX_COLOR_DEFAULT;
        private static final Scalar CENTER_TO_TARGET_LINE_COLOR = BlueTaskCApi.CENTER_TO_TARGET_LINE_COLOR;
        private static final int CONNECTION_LINE_THICKNESS = BlueTaskCApi.CONNECTION_LINE_THICKNESS;
        private static final Scalar BLOCK_CENTER_CIRCLE_COLOR = BlueTaskCApi.BLOCK_CENTER_CIRCLE_COLOR;
        private static final Scalar TEXT_COLOR = BlueTaskCApi.TEXT_COLOR;
        private static final Scalar GRASPABLE_ANGLE_LINE_COLOR = BlueTaskCApi.GRASPABLE_ANGLE_LINE_COLOR;
        private static final int GRASPABLE_ANGLE_LINE_THICKNESS = BlueTaskCApi.GRASPABLE_ANGLE_LINE_THICKNESS;

        // --- Pipeline Members ---
        private final Mat rgbImage = new Mat();
        private final Mat hsvImage = new Mat();
        private final Mat BLUEMask = new Mat(); // Renamed for clarity, still uses Blue ranges
        ...
        public Mat processFrame(Mat input) {
            ...
            // 1. Rotation (if configuBlue)
            if (IMAGE_ROTATE_180) {
                Core.rotate(input, input, Core.ROTATE_180);
                // Input buffer is now rotated, displayOutput points to it
            }

            // 2. Downscaling (if configuBlue) - Process smaller, draw on original/rotated
            ...
            for (MatOfPoint contour : contours) {
                ...
                if (area >= MIN_SIZE_PIXELS && area <= MAX_SIZE_PIXELS) {
                    ...
                    // Calculate distance squaBlue from operational center
                    ...
                    currentFrameCubes.add(new DetectedCube("Blue", // Color label (using Blue ranges)
            ...
            // 8. Update ShaBlue State (Thread-Safe)
            detectedCubesLock.lock();
            try {
            ...
            // Draw detected cubes (boxes, centers, info) - use `allDetectedCubes` from shaBlue state for safety
            detectedCubesLock.lock();
            try{
        ...
        private void drawDetections(Mat displayOutput, List<DetectedCube> cubes, DetectedCube selected, Point drawOpCenter, double drawScale) {
            ...
            for (DetectedCube cube : cubes) {
                ...
                if (cube == selected) {
                    boxColor = BOX_COLOR_SELECTED; // Blue for the selected cube
