...
public class TaskBApi {
    ...
    private static final double CIRCLE_CENTER_OFFSET_CM_Y = 4.5;
    private static final double ARM_BACK_CM = 11.9;
    private static final double ERROR_CM = 5.0;
    private static final double GRASPABLE_ANGLE_DEG = 110.0;
    private static final List<Scalar[]> RED_HSV_RANGES = List.of(
            new Scalar[]{new Scalar(20, 80, 170), new Scalar(40, 255, 255)},
            new Scalar[]{new Scalar(20, 80, 170), new Scalar(40, 255, 255)}
    );
    ...
    private static final double SLIDE_MOVE_SPEED_STAGE_3 = 0.45;  // Speed for stage 3 (19000 -> 24945)
    ...
    public boolean initialize(HardwareMap hwMap, Telemetry tel) {
        if (isInitialized) {
            if (tel != null) tel.addLine("TaskBApi: 已初始化。");
            return true;
        }
    ...
    private void logInfo(String message) {
        if (telemetry != null) {
            telemetry.addLine("TaskBApi: " + message);
            telemetry.update(); // Consider updating less frequently if performance is an issue
        }
        System.out.println("TaskBApi INFO: " + message); // Log to console/logcat
    }

    private void logError(String message) {
        if (telemetry != null) {
            telemetry.addLine("!!! TaskBApi 错误: " + message);
            telemetry.update(); // Update immediately for errors
        }
        System.err.println("!!! TaskBApi 错误: " + message); // Log to error stream
    }
    ...
    public static class BLUEDetectionPipeline extends OpenCvPipeline {
        // --- Constants copied from TaskBApi ---
        // Ensure these match the outer class's constants *exactly*
        private static final boolean IMAGE_ROTATE_180 = TaskBApi.IMAGE_ROTATE_180;
        private static final double DOWNSCALE_FACTOR = TaskBApi.DOWNSCALE_FACTOR;
        private static final double ARM_BACK_CM = TaskBApi.ARM_BACK_CM;
        private static final double ERROR_CM = TaskBApi.ERROR_CM;
        private static final double CIRCLE_CENTER_OFFSET_CM_X = TaskBApi.CIRCLE_CENTER_OFFSET_CM_X;
        private static final double CIRCLE_CENTER_OFFSET_CM_Y = TaskBApi.CIRCLE_CENTER_OFFSET_CM_Y;
        private static final double GRASPABLE_ANGLE_DEG = TaskBApi.GRASPABLE_ANGLE_DEG;
        private static final double PIXELS_TO_CM_RATIO = TaskBApi.PIXELS_TO_CM_RATIO;
        private static final double MIN_SIZE_PIXELS = TaskBApi.MIN_SIZE_PIXELS;
        private static final double MAX_SIZE_PIXELS = TaskBApi.MAX_SIZE_PIXELS;
        private static final List<Scalar[]> BLUE_HSV_RANGES = TaskBApi.RED_HSV_RANGES; // Name adjusted for clarity, uses RED ranges
        private static final Scalar CIRCLE_COLOR = TaskBApi.CIRCLE_COLOR;
        private static final int CIRCLE_THICKNESS = TaskBApi.CIRCLE_THICKNESS;
        private static final Scalar PERSISTENT_LINE_COLOR = TaskBApi.PERSISTENT_LINE_COLOR;
        private static final int PERSISTENT_LINE_THICKNESS = TaskBApi.PERSISTENT_LINE_THICKNESS;
        private static final Scalar BOX_COLOR_SELECTED = TaskBApi.BOX_COLOR_SELECTED;
        private static final Scalar BOX_COLOR_DEFAULT = TaskBApi.BOX_COLOR_DEFAULT;
        private static final Scalar CENTER_TO_TARGET_LINE_COLOR = TaskBApi.CENTER_TO_TARGET_LINE_COLOR;
        private static final int CONNECTION_LINE_THICKNESS = TaskBApi.CONNECTION_LINE_THICKNESS;
        private static final Scalar BLOCK_CENTER_CIRCLE_COLOR = TaskBApi.BLOCK_CENTER_CIRCLE_COLOR;
        private static final Scalar TEXT_COLOR = TaskBApi.TEXT_COLOR;
        private static final Scalar GRASPABLE_ANGLE_LINE_COLOR = TaskBApi.GRASPABLE_ANGLE_LINE_COLOR;
        private static final int GRASPABLE_ANGLE_LINE_THICKNESS = TaskBApi.GRASPABLE_ANGLE_LINE_THICKNESS;
